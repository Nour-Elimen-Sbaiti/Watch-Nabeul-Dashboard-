...
// The backend sends short/raw classification labels (e.g. "Agriculture",
// "Vegetation"). This maps them to the display text we actually want
// shown in the popup, without touching the underlying keys used for
// color/icon/description lookups.
const CLASSIFICATION_DISPLAY_NAMES = {
  'Agriculture': 'Agricultural Area',
  'Agricultural_area': 'Agricultural Area',
  'Vegetation': 'Forest',
};
const displayName = (name) => CLASSIFICATION_DISPLAY_NAMES[name] || name;
...
function buildPopupHTML(pixelData, layerType, selectedYear, modelVariant = "cnn", accuracy = null) {
  ...
  // Display-friendly version of the classification label. classification_key
  // is left untouched below, since colorMap / classDescription / icons all
  // look up by that raw key.
  const classificationDisplay = displayName(classification);
    ...
    body = `
      ...
      <div style="margin-bottom:8px">
        ${row("Classification Class",
          `<span style="display:flex;align-items:center;font-size:12px">${dot(classColor)}<strong>${classificationDisplay || "Unknown"}</strong></span>`,
          classColor)}
...
/* ─── RasterManager ──────────────────────────────────────────── */
function RasterManager({ layerType, selectedYear, rasters, modelVariant }) {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    tileLayerRef.current = L.tileLayer(raster.resolvedUrl, { opacity: 0.9, pane: 'tilePane' }).addTo(map);
    return () => { if (tileLayerRef.current) { map.removeLayer(tileLayerRef.current); tileLayerRef.current = null; } };
  }, [layerType, selectedYear, rasters, modelVariant, map]);
